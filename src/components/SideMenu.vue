    ...
    <div class="content">
      <div v-if="!isUserOpenned" class="legend">
        ...
        <div class="legend__chart">
          ...
          <!-- По аналогии с ситуацией в Map.vue, гарантируем (v-once), что внешним
          видом графика мы управляем самостоятельно, без вмешательства vue-->
          <PieChart ref="chart" v-once />
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      legend: [],
      // formatDate не имеет зависимостей от props или data,
      // поэтому её размещение как computed (как делалось в презентации
      // к воркшопу) не имеет смысла: оно никогда не обновится.
      // Объявим ее здесь и будем обновлять при регенерации графика.
      formatedDate: format(new Date(), "dd.MM.yyyy hh:mm"),
    };
  },
  created() {
    this.loadLegend();
  ...
  mounted() {
    // Все соображения аналогичны тем, что использованы в Map.vue
    this.$nextTick(() => {
      this.makeChart();
      this.$watch("legend", this.makeChart, {
        deep: true,
      });
      this.$watch("isUserOpenned", this.makeChart);
    });
  },
  methods: {
    loadLegend() {
